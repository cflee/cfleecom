...
This allows hosts' requests for recently requested packages to be answered quickly, which is particularly useful when patching hosts.
As a pull-through cache that does APT-aware cleanups, only the package files that have been requested and are still current are retained on disk, which saves a lot of space compared to mirroring the full Ubuntu/Debian repositories using apt-mirror.

APT just treats it as a regular HTTP proxy, so there's no special protocol to communicate with it.
In particular, apt-cacher-ng's ["tell-me-what-you-need"](https://www.unix-ag.uni-kl.de/~bloch/acng/html/howtos.html#ssluse) method is used to access external repos over HTTPS, but that's not really material to this issue.
...
Other repositories like [Docker Engine](https://docs.docker.com/install/linux/docker-ce/ubuntu/#install-using-the-repository) that we were accessing over HTTPS for whatever reason weren't impacted, so we know it's not related to HTTPS-specific quirks in apt-cacher-ng.

Naturally, the problem goes away temporarily by deleting the corrupt files from apt-cacher-ng's cache, until the next repository update.
...
> This avoids the problem of getting a signature file that doesn't match the Release file, if the repository is updated just as a client is downloading the two files.
...
The corrupt file with that sixth dash was the "correct" size at 4867 bytes; it is one byte longer than the previous version of the file at 4866 bytes, because the day in the date field went from single-digit (`4`) to double-digit (`11`).
...
After chasing down several other red herrings, the breakthrough came when we found an apt-cacher-ng instance with an uncorrupted previous copy of the `main/binary-amd64/Packages.bz2` file, and collected copies of that file before/after an APT client requested for it.

Since it's a bzip2 file, using [HexFiend](https://ridiculousfish.com/hexfiend/) (though you could also diff the text output of `od` or `xxd`) to inspect a binary diff of the corrupt file and a fresh intact file downloaded directly from upstream yielded nothing interesting as it was pretty much all different, except the tail bit of the file was fully intact.

But upon diffing the intact old file and the corrupt file, it was immediately obvious that corrupt file was composed of the old file + the last 192 bytes or so of the new file, as if bytes from the new file were appended to reach the length of the new file.
That is consistent with the sixth dash seen in the InRelease file, since the newer version was one byte longer, and the last byte was a dash.
...
I don't really know why APT used to do that over `If-Modified-Since:`, but this single-byte thing seemed to be removed and replaced with proper 416 response handling back in Sep 2013 in [78c72d0c](https://salsa.debian.org/apt-team/apt/commit/78c72d0ce22e00b194251445aae306df357d5c1a) (v0.9.12).
Now the comments in APT seem to indicate that conditional range requests should only be used to resume partial downloads, and checking freshness should be accomplished using `If-Modified-Since:`.
...
In that case, when given a partial request, it will always respond with either a 206 Partial Content (with some subset of bytes), or 416 Range Not Satisfiable (due to the file being shorter than the client thinks it is), and never with a 200 OK (with the full contents of the file).
...
The last modified timestamp of the file doesn't match the `If-Range:` value, but yet it returns a 206 response with the final 2 bytes of the file.
...
However, apt-cacher-ng's HTTP client doesn't check that, and assumes that if the server returns a 206 response, it is behaving as expected.
Then it seems to append the received data to the file, starting with overwriting the last byte in the file with the first received byte, leading to the weird append behaviour.
...
If we look at the Server header, it identifies itself as running version 1.6.9 of the [Monkey](http://monkey-project.com/).

Unfortunately, that bit of HTTP seems to basically have never been implemented.
If we look at the HTTP [request header enum](https://github.com/monkey/monkey/blob/v1.6.9/include/monkey/mk_http_parser.h#L92-L120) or the [request header parsing](https://github.com/monkey/monkey/blob/v1.6.9/mk_server/mk_http.c#L151-L157), it understands the `Range:` (and `If-Modified-Since:`) header but not `If-Range:`.
...
But while browsing the GitHub repo further, I thought the Monkey server's author's username looked familiar -- [edsiper](https://github.com/edsiper) is the creator and maintainer of Monkey _and_ of Fluent Bit, looking at the [GitHub contributors page for Monkey](https://github.com/monkey/monkey/graphs/contributors) and [for Fluent Bit](https://github.com/fluent/fluent-bit/graphs/contributors).
